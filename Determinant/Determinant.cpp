...
cout << "Zadejte řád matice:\n";
cin >> order;
matrix = new long[(order) * (order)];

cout << "Zadejte elementy matice:\n";
for (j = 0; j < order; j++)
   for (i = 0; i < order; i++)
      cin >> matrix[i + (order * j)];

cout << "Zadali jste matici: \n";     
   for (j = 0; j < order; j++) {  
      for (i = 0; i < order; i++)
         cout << matrix[i + (order * j)] <<" ";
      cout<<endl;
      }
cout<<"Determinant matice je: "<< determinant(order, matrix) << " ";

return 0;} // Autor dovoluje tento program šířit pouze s uvedením jména autora Ing. Pavel Florián Ph.D.
