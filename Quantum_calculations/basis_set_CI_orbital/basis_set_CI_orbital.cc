...
if (success == 0)
    ...
    for (i = 0; i < count_electrons; i++)
        if (count_shells < Slater_basis_set_calculations_DFT.results.n[i])
            count_shells = Slater_basis_set_calculations_DFT.results.n[i];
    Hamiltonian = Slater_basis_set_calculations_DFT.Execute_Basis_set_creation(max_iterations, fidelity, lenght_order,
    false, &values, count_shells, 3);
...
cout<< "basis functions: / základní funkce: / Basisfunktionen: / базисные функции:"<<endl;
for  (i = 0; i < Slater_basis_set_calculations_DFT.n_extended.size(); i++)
    {
    cout<<"n = "<< Slater_basis_set_calculations_DFT.n_extended[i] <<", l = "<< Slater_basis_set_calculations_DFT.l_extended[i] <<
    ", m = " << Slater_basis_set_calculations_DFT.m_extended[i] << ", s = " << Slater_basis_set_calculations_DFT.s_extended[i] <<
    ", Z_eff = " << Slater_basis_set_calculations_DFT.Slater_basis_exponents_extended[i] << endl; 
    }
/*
for  (i = 0; i < count_electrons; i++)
    {
