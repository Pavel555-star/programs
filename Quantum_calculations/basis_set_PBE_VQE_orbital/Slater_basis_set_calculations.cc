...
int Slater_basis_set_calculations<T>::basis_set_Determinant_solver(unsigned int order, T* pointer)
    ...
    range_coefficient = (max_row/order) * (1 + 1.00/(5000.00));
    try {
    ...
    # pragma omp parallel
        ...
        for (i = 0; i < order * 8; i++)
            {
            basis_set_Determinant_set(order, pointer, 1250, range_coefficient * (0.125 * i - order),
